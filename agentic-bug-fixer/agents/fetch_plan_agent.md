...
query {
  viewer {
    assignedIssues(
      first: 1
      filter: { state: { name: { eq: "Backlog" } }, priority: { gte: 1 } }
      orderBy: createdAt
    ) {
      nodes {
        id
...
2. Writes the bug details to `tasks/active_bug.md`
3. Logs the analysis to `logs/system_logs.md`
...
2. Writes bug details to `tasks/active_bug.md`
3. Updates `state/workflow_state.md` (via Orchestrator)
