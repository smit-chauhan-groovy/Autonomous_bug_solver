...
> [!IMPORTANT]
> **NO LOCAL STORAGE**: Do not write bug details or API responses to local files. Pass the extracted data directly to the Orchestrator via terminal output.
...
2. **Query (Node.js Fallback):**
    ...
    - 🚨 **CRITICAL DEMO REQUIREMENT (Exact Script)**: Do NOT hallucinate the GraphQL query or filters. You MUST write exactly this logic to ensure it fetches the most recent active bug without failing:
      ...
      .then(data => {
         ...
         if (!issue) { 
           console.log("ERROR: No open issues found"); 
           process.exit(0); // Exit with 0 but print the exact error message for the Orchestrator to catch
         }
      ...
      ```
3. **Execute Script**: Run the node script (e.g., `node --env-file=.env /tmp/fetch_linear.js`) to extract the actual `Bug ID` and `Bug details`.
